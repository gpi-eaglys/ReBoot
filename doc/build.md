...
```
git clone --depth 1 --branch v0.8.9 https://github.com/openfheorg/openfhe-python.git build/openfhe-python
PYBIND11_DIR=$(.venv/bin/python -m pybind11 --cmakedir)
CMAKE_PREFIX_PATH="/opt/openfhe/v1.2.1/install:$PYBIND11_DIR" \
  uv pip install -e ./build/openfhe-python --python .venv/bin/python
```
...
- `build/` is gitignored; it holds every build artifact for this repo (CMake build trees, vendored build-time source checkouts like this one) — none of it is repo source.