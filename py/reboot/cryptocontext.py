...
class CryptoContext:
    ...
    def __init__(
        ...
        reboot_py.EncryptedValue.set_context(cc)
        reboot_py.EncryptedValue.generate_sum_keys(self.kp, self.col_size)
    ...
    def encode_array(self, array: NDArray[np.float32], level: int = 0) -> NDArray[PT]:
        ...
        return reboot_py.encode_array(array, self.cc, level)

    def encrypt_array(
        self, array: NDArray[np.float32], level: int = 0
    ) -> NDArray[reboot_py.EncryptedValue]:
        """
        ...
        return reboot_py.encrypt_array(array, self.cc, self.kp, level)

    def decrypt_array(
        self,
        array: NDArray[reboot_py.EncryptedValue] | reboot_py.EncryptedValue,
        num_slots: int,
    ) -> NDArray[np.float32]:
        """
        ...
        """
        if isinstance(array, reboot_py.EncryptedValue):
            array = np.array([array])
        return reboot_py.decrypt_array(array, self.cc, self.kp, num_slots).astype(
            np.float32
        )

    def get_precision(self, array: NDArray[reboot_py.EncryptedValue]) -> str:
        """
        ...
        def __get_precision(value: reboot_py.EncryptedValue) -> float:
    ...
    def get_level(self, array: NDArray[reboot_py.EncryptedValue]) -> int:
        """
    ...
    def is_encrypted(self, array: NDArray[reboot_py.EncryptedValue]) -> bool:
        """
        ...
        """
        return isinstance(array.item(0), reboot_py.EncryptedValue)
    ...
    def repeat_and_encrypt(self, x, debug: bool = False):
        ...
        encrypted_x = reboot_py.encrypt_array(repeated_x, self.cc, self.kp)
        return encrypted_x
    ...
    def expand_and_encrypt(self, x, debug: bool = False):
        ...
        encrypted_x = reboot_py.encrypt_array(expanded_x, self.cc, self.kp)
        return encrypted_x
