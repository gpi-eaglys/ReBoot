...
from lib.utils.enums import OptimizerName
...
class CKKSPackedLinear(Linear):
    ...
    def encrypt_layer(self, cc) -> None:
        ...
        self.weights = reboot_cpp.EncryptedValue(
    ...
    def forward(self, x: Array) -> Array:
        if self.training:
            ...
            self.last_input = x.copy()
        if self.row_packing:
            out = reboot_cpp.row_packing_forward(x, self.weights, self.col_size)
        else:
            out = reboot_cpp.col_packing_forward(x, self.weights, self.col_size)
    ...
    def compute_gradients(self, delta: Array) -> tuple[Array, Array, Array]:
        ...
        if self.propagate_backward:
            # Compute the new delta, i.e., the gradient towards the input
            if self.row_packing:
                new_delta = reboot_cpp.row_packing_backward(
                    delta, self.weights, self.col_size
                )
            else:
                new_delta = reboot_cpp.col_packing_backward(
                    delta, self.weights, self.col_size
                )

        weight_gradient, _ = None, None
        if self.trainable:
            weight_gradient = reboot_cpp.packing_weight_update(self.last_input, delta)
