...
class Square(Activation):
    ...
    def forward(self, x: Array) -> Array:
        super().forward(x)
        if self.encrypted:
            out = reboot_cpp.square_forward(x)
        else:
            out = x * x
    ...
    def backward(self, delta: Array, lr: float) -> Array:
        if self.encrypted:
            out = reboot_cpp.square_backward(delta, self.last_input)
        else:
...
class PolyReLU(Activation):
    ...
    def forward(self, x: Array) -> Array:
        super().forward(x)
        if self.encrypted:
            out = reboot_cpp.poly_relu_forward(x)
        else:
            out = x * x + x
        self.log_stats(out, "Forward")
        return out
    ...
    def backward(self, delta: Array, lr: float) -> Array:
        if self.encrypted:
            out = reboot_cpp.poly_relu_backward(delta, self.last_input)
        else:
