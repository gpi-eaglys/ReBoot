...
if __name__ == "__main__":
    ...
    # Encryption
    file_path = f"fhe_config_mlp{configs['num_fc_layers'] - 1}.yaml"
    ...
    for epoch_id in range(configs["num_epochs"]):
        ...
        for batch_id in range(n_train_batches):
            ...
            save_accuracies_csv(epoch_id, batch_id, acc_plain_batch, acc_enc_batch, configs["seed"])
            
            padding_dict = compute_zero_weights(encrypted_model, cc)
            repeated_dict = check_repeated_values(encrypted_model, cc)
            ...
            batch_wandb_log(
                ...
                acc_enc_batch,
                step_time,
                differences_dict | means_dict | precisions_dict | padding_dict | repeated_dict,
            )
    ...
    print(f"Final train loss: {loss_enc:.5f}")
    print(f"Final train accuracy: {acc_enc:.2%}")
